...
import { getContactRecaptchaToken, type RecaptchaClient } from './contact-recaptcha';
...
export default function ContactForm({ definition }: { definition: ContactFormDefinition }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setStatus(null);
    const recaptcha = (window as Window & { grecaptcha?: RecaptchaClient }).grecaptcha;
    if (!siteKey || !recaptcha) {
      setStatus({ type: 'error', message: 'Security verification is unavailable. Please try again shortly.' });
      return;
    }

    setIsSubmitting(true);
    const recaptchaToken = await getContactRecaptchaToken(recaptcha, siteKey).catch(() => null);
    if (!recaptchaToken) {
      setIsSubmitting(false);
      setStatus({ type: 'error', message: 'Security verification failed. Please try again.' });
      return;
    }
    const response = await fetch('/api/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...values, website: '', recaptchaToken }) }).catch(() => null);
  ...
  return <>{siteKey ? <Script src={`https://www.google.com/recaptcha/api.js?render=${encodeURIComponent(siteKey)}`} strategy="afterInteractive" /> : null}
  <form className="mt-10 grid gap-6" noValidate onSubmit={submit}>
