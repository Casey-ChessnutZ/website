...
import { getFooterNavigationGroups, getNavigationGroups } from '@/app/lib/navigation';

export default async function SiteLayout({ children }: { children: ReactNode }) {
  const { isEnabled: preview } = await draftMode();
  const settings = await getSiteSettings(preview);
  const siteName = settings.siteName ?? 'ChessNutZ';
  const navigationGroups = getNavigationGroups(settings.navigationConfig);
  const footerNavigationGroups = getFooterNavigationGroups(settings.footerNavigationConfig);
  ...
  return (
    <div className="min-h-dvh">
      ...
      <SiteHeader siteName={siteName} groups={navigationGroups} />
      {children}
      <SiteFooter siteName={siteName} footerText={settings.footerText} groups={footerNavigationGroups} />
    </div>
  );
}
