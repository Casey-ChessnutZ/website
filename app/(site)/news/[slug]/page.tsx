...
export async function generateMetadata({ params }: NewsPageProps): Promise<Metadata> { const { slug } = await params; const entry = await getPublishedNewsBySlug(slug); return getPageMetadata(entry?.title ?? 'News', entry?.summary, `/news/${slug}`); }

export default async function NewsDetailPage({ params }: NewsPageProps) {
  const { slug } = await params;
  const entry = await getPublishedNewsBySlug(slug);
  if (!entry) notFound();
